        ...
        void* args)
{
    ...
    printf("time_offset is %d\n",session->time_offset);
}

static void time_sync_entry(void *arg)
{
...
static void time_sync(int argc, char** argv)
{
    rt_thread_t tid;

    tid = rt_thread_create("tim_sync",
                            time_sync_entry,
                            RT_NULL,
                            8192,
                            25,
    ...
    {
        rt_kprintf("Failed to create tim_sync thread!\n");
    }

    return;
}
MSH_CMD_EXPORT(time_sync, micro-ros time sync demo.);
